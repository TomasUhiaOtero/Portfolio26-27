...
/**
 * The projects section's background: a seeded diagonal grid of diamonds
 * with travelling ignition pulses — user-supplied Canvas 2D component,
 * ported from TypeScript with the minimum changes needed to fit this
 * project: theme comes from `document.documentElement.dataset.theme`
 * (this project's own `ThemeProvider`, not a `.dark` class), the ground
 * colour matches `--bg` exactly, and the wrapper follows this project's
 * `aria-hidden` convention for decorative canvases (see `LazyCanvas.jsx`)
 * instead of `role="img"`. Every animation constant, the mask gradient,
 * the alpha lookup tables and the ignition/decay maths are untouched.
 *
 * Deliberately not routed through `LazyCanvas`: that wrapper exists to
 * keep the `three`/`@react-three` chunk out of the initial bundle (see
 * its own docblock) — this component has no such dependency, and it
 * already owns its play/pause (IntersectionObserver + `visibilitychange`)
 * and its reduced-motion fallback (a single static frame, not a poster
 * swap), so wrapping it in that machinery would only duplicate it.
 */
...
const LIGHT = {
  // Matches this project's `--bg` (light) exactly, not the component's
  // original `#FAF8F5` — everything else about the palette is untouched.
  ground: "#FBFBFD",
  ink: "14,14,16",
  line: 0.34,
  pulse: 0.5,
...
const clampInt = (value, low, high) => Math.round(clamp(value, low, high))
...
function gridToScreen(transform, x, y) {
  return {
...
function screenToGrid(transform, x, y) {
...
function buildField(W, H, seed) {
  ...
  for (let i = 0; i < EVENT_COUNT; i++) {
    ...
    if (i % 3 === 1 && events.length > 0) {
      ...
      const point = gridToScreen(transform, x0 + position.col * cell, y0 + position.row * cell)
      if (
        point.x < 0 ||
        point.x > W ||
        point.y < 0 ||
...
function addStarPath(context, x, y, radius, inner) {
...
export default function DiamondGrid({ className = "", seed = 1337 }) {
  ...
  useEffect(() => {
    ...
    // This project's own theme flag (`ThemeProvider.jsx` sets this, never
    // a `.dark` class) — defaults to dark, matching `readStoredTheme`'s
    // own fallback.
    ...
    if (!baseCtx || !liveCtx || !maskCtx || !starCtx) return undefined
    ...
    const styleAt = (alpha) => styles()[Math.round(clamp(alpha, 0, 1) * 255)]

    const sizeSurface = (surface, width, height) => {
      surface.width = Math.max(1, Math.round(width * dpr))
      surface.height = Math.max(1, Math.round(height * dpr))
    }

    const setGridTransform = (target, f) => {
    ...
    const applyMask = (target, surface) => {
    ...
    const paintIgnition = (event, time) => {
      if (!field) return
      ...
      liveCtx.drawImage(starCanvas, x - STAR_SIZE * 0.5, y - STAR_SIZE * 0.5, STAR_SIZE, STAR_SIZE)
    ...
    // `isDark()` read above (inside `rebuild` -> `rebuildCaches`) can race
    // ThemeProvider's own `data-theme` mutation: effects run child-first,
    // so on first mount this component's effect runs before the ancestor
    // ThemeProvider's does. Same defer every other themed scene in this
    // project uses (see e.g. HeroField.jsx) — re-check one frame later and
    // only rebuild if the raced guess was actually wrong.
    const themeRaf = requestAnimationFrame(() => {
      const next = isDark()
      if (next !== dark) {
        dark = next
        rebuildCaches()
  ...
  return (
    // No `position` baked in here — the caller's `className` decides it
    // (Work.jsx passes `absolute inset-0`). `position: relative` and
    // `position: absolute` both set the same CSS property, so hardcoding
    // one here would fight whichever the caller passes: Tailwind resolves
    // that conflict by stylesheet order, not by the order classes appear
    // in the string, so it silently picks one — starving `<canvas>` of a
    // real containing block and collapsing its height to near zero (this
    // shipped broken once already; see the fix that added this note).
    <div ref={hostRef} aria-hidden="true" className={`overflow-hidden bg-bg ${className}`}>
